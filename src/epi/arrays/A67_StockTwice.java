...
/**
 * Write a program that computes the max profit that can be made by buying and
 * selling a share at most twice. The second buy must be made after the first
 * sale.
 * 
 * @author minhhoang
 *
...
//		System.out.println(getMaxProfit(prices, firstMaxIndex));
	...
	public static int getMaxProfit(int[] prices, int indexToExclude) {
		...
		for (int i = 0; i < prices.length; i++) {
			...
			if (i != indexToExclude) minSoFar = Math.min(prices[i], minSoFar);
		}
