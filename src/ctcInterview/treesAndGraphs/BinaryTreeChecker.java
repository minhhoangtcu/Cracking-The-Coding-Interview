...
public class BinaryTreeChecker {
	...
	/**
	 * Check for binary tree in in-order traversal (because in-order will always smallest -> biggest).
	...
	public static boolean isBinaryTree(Tree tree) {
		 ...
		 * -> push all left node into the stack. 						(1)
		 * -> when we reach the left-most node, print it out 			(left)
		 * -> pop the stack, or go up the tree, print the node out 		(mid)
		 * -> add the right child of the node into the program 			(right)
		 * -> push all left node into the stack like in (1) then repeat.		
		 */
